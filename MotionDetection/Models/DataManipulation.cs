...
namespace MotionDetection.Models {
	class DataManipulation {
		...
		public static double[][] Modulo(CircularBufferMatrix<double> A) {
			double[][] B = new double[A.SensorType][];
			for(int i = 0; i<A.SensorType; i++) {
		...
		public static double[][][] Smoothing(CircularBufferMatrix<double> A) {
			double[][][] B = new double[A.SensorNumber][][];//, A.SensorType, A.Time];
			int s = 5; // smoothing size 2*s+1
			for(int i = 0; i<A.SensorNumber; i++) {
				B[i] = new double[A.SensorType][];
				for (int j = 0; j<A.SensorType; j++) {
					...
					for (int k = 0; k<A.Time; k++) {
						if (k>10 && (k+10)<A.Time) {
							...
							if (k<s) {
								...
								for (int tmp = k - s; tmp<A.Time; tmp++) {
									B[i][j][k] += A[i, j, tmp];
								}
		...
		public static double[][] DifferenceQuotient(CircularBufferMatrix<double> A, int Sensor) { // A = sampwin[,] o 
			double[][] B = new double[A.SensorType][];//, A.Time];
			for(int i = 0; i<A.SensorType; i++) {
				B[i] = new double[A.Time];
				for (int j = 0; j<A.Time-1; j++) {
		...
		public static double[][][] Eulero(CircularBufferMatrix<double> A) {
			/*
				...
				2 = Yaw (i.e. ψ).
			*/
			double[][][] B = new double[3][][]; //,A.SensorType,A.Time];
